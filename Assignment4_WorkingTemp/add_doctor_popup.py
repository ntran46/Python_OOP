...
class AddDoctorPopup(tk.Frame):
    ...
    def __init__(self, parent, close_callback):
        ...
        """ ID will be taken from DB, ID is auto-generated by SQLLite"""
        # ttk.Label(self, text="ID:").grid(row=1, column=1)
        # self.id = ttk.Entry(self)
        # self.id.grid(row=1, column=2)
        ttk.Label(self, text="First Name:").grid(row=1, column=1)
        self.first_name = ttk.Entry(self)
        self.first_name.grid(row=1, column=2)
        ttk.Label(self, text="Last Name:").grid(row=2, column=1)
        self.last_name = ttk.Entry(self)
        self.last_name.grid(row=2, column=2)
        ttk.Label(self, text="Date of Birth:").grid(row=3, column=1)
        self.date_of_birth = ttk.Entry(self)
        self.date_of_birth.grid(row=3, column=2)
        ttk.Label(self, text="Address:").grid(row=4, column=1)
        self.address = ttk.Entry(self)
        self.address.grid(row=4, column=2)
        ttk.Label(self, text="Is released:").grid(row=5, column=1)
        self.is_released = ttk.Entry(self)
        self.is_released.grid(row=5, column=2)
        ttk.Label(self, text="Office Number:").grid(row=6, column=1)
        self.office_num = ttk.Entry(self)
        self.office_num.grid(row=6, column=2)
        ttk.Label(self, text="Income:").grid(row=7, column=1)
        self.income = ttk.Entry(self)
        self.income.grid(row=7, column=2)

        ttk.Button(self, text="Submit", command=self._submit_cb).grid(row=8, column=1, pady=20)
